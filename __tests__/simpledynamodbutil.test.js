...
describe('SimpleDynamoDBUtil', () => {
  ...
  describe('createNewItem', () => {
    test('should create item successfully, when valid arguments passed', async () => {
      ...
      await simpleDDBUtil.createNewItem('users', userItem)
    ...
    test('should throw an error, when non-existent table passed', async () => {
      expect.assertions(1)
      const userItem = { id: '1', hello: 'world' }
      await expect(simpleDDBUtil.createNewItem('users-invalid', userItem, 'id')).rejects.toThrowError('Cannot do operations on a non-existent table')
    }) // test
    test('should throw an error, when invalid userItem passed', async () => {
      expect.assertions(1)
      const userItem = { id: '', hello: 'world' }
      await expect(simpleDDBUtil.createNewItem('users', userItem, 'id')).rejects.toThrowError('Invalid attribute value type')
    }) // test
    test('should create new item and return old item-json data, when invalid pkAttributeName passed and similar item already exist', async () => {
      expect.assertions(1)
      const userItem = { id: '1', hello: 'world' }
      const { Attributes } = await simpleDDBUtil.createNewItem('users', userItem, 'idinvalidpk')
    ...
    test('should overwrite existing item, when similar item already exist', async () => {
      ...
      await simpleDDBUtil.createNewItem('users', updatedUserItem) // overwrite an existing item
    ...
    test('should prevent overwrite to existing item and throw an error, when valid pkAttributeName passed and similar item already exist', async () => {
      ...
      await expect(simpleDDBUtil.createNewItem('users', updatedUserItem, 'id')).rejects.toThrowError('The conditional request failed')
    }) // test
  }) // describe('createNewItem')
}) // describe('SimpleDynamoDBUtil')
